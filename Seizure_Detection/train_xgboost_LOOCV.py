...
for sec in [1,2,4,8]:
    print(f"Processing {sec}-second files")

    for patient in patients:
        print(f"Processing patient: {patient}")
        # Retrieve list of feature files for each patient
        files = os.listdir(os.path.join(data_save, patient))
        ...
        for i in range(num_trials):
            cnt = 0
            if(not os.path.exists("results/subject_specific/" + patient + "/" + trials[i] + "_xgboost_all_"+str(sec)+"s.pkl")):
                for j in range(num_trials):
                    if(j != i):
                        ...
                        if(patient == 'chb17'):
                            if(trials[j] == '63'):
                                X_test = np.load(data_save + patient + '/' + patient + 'b_' + trials[j] + '_features_'+str(sec)+'s.npy')
                                y_test = np.load(data_save + patient + '/' + patient + 'b_' + trials[j] + '_labels_'+str(sec)+'s.npy')
                            else:
                                X_test = np.load(data_save + patient + '/' + patient + 'a_' + trials[j] + '_features_'+str(sec)+'s.npy')
                                y_test = np.load(data_save + patient + '/' + patient + 'a_' + trials[j] + '_labels_'+str(sec)+'s.npy')
                        else:    
                            X_test = np.load(data_save + patient + '/' + patient + '_' + trials[j] + '_features_'+str(sec)+'s.npy')
                            y_test = np.load(data_save + patient + '/' + patient + '_' + trials[j] + '_labels_'+str(sec)+'s.npy')
                ...
                df.to_pickle("results/subject_specific/" + patient + "/" + trials[i] + "_xgboost_all_"+str(sec)+"s.pkl")